...
const SignupStaff = () => {
		...
		try {
			...
			if (data[1] === data[2]) {
				if (data[0] && data[1] && data[3] && data[4] && data[5]) {
					const response = await fetch('https://odd22.onrender.com/signup_staff', {
						method: 'POST',
						headers: {
	...
	return (
		<div className='h-[100vh]'>
			<div className='absolute top-[130px] right-[50px] left-[300px] max-800:left-[50px] bottom-[50px] max-800:border-none border-[1px] border-black rounded flex justify-center items-center'>
							...
							<form action="" onSubmit={handleSubmit}>
								{inputSignup.map((s, i) => {
									return (i != 4) ? <>
										<input type={s.placeholder == 'Email' ? 'email' : 'text'} placeholder={s.placeholder}
											...
											className='w-full px-[10px] py-[5px] mb-[10px] rounded focus:outline-none bg-white text-black max-800:border-b-2 md:border-2 border-gray'
										/> <br />
									</> : null
								})}
								...
								<div className='flex justify-start items-center'>
									...
									<select
										...
										className='outline-none relative w-[50%] md:rounded  bg-white border-2 border-gray px-[5px] py-[5px]'
