...
const WelcomeStaff = () => {
	...
	return (
		<div className='md:px-[200px] px-[20px]'>
			...
			<div className='flex justify-center items-center '>
				<div className='md:flex justify-center items-center gap-5 mt-[20px] w-full'>
					{SELECTED_PAGES.map((s) => {
						if (s.name == 'SignupStaff' || s.name == 'LoginStaff') {
							return <button className={` ${s.name == 'SignupStaff' ? 'bg-white text-[#04314C] border-2 border-[#04314C]' : 'bg-[#04314C] text-white'} rounded w-full h-[50px] font-medium px-[5px] py-[5px] mb-[10px] `}
								onClick={() => {
			...
			<div>
				<div className='px-[20px] mt-[20px] border-[1px]'>
					...
					<ul className='relative grid grid-cols-3 max-[600px]:grid-cols-1 gap-3 justify-center items-center w-full'>
						{sectionInfo.map((s) => {
