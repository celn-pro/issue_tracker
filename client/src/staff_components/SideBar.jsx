		...
		<div className='border-[1px] border-black px-[20px] py-[20px] rounded h-full'>
			<div>
				{ADMIN_SIDEBAR.map((s)=>{
					return (s.name =='Profile'|| s.name =='Aunthenticate' || s.name =='Stats' || s.name == 'WorkBoard'|| s.name == 'Delegate')?
						<div className={` ${selectedAdminSideBar == s.name ? 'bg-yellow-300' : ''} ${s.name=='Stats'||s.name =='Delegate'|| s.name =='Aunthenticate'?'hidden':''} px-[10px] flex justify-start items-center gap-5 mb-[10px] cursor-pointer rounded `} onClick={() => toggleSelectedAdminSideBar(s.name)}>{s.icon} <div className='inline-block'>{s.name}</div></div>:null
				})}
			</div>
		  ...
		  <div className='bg-white border-[1px] hidden max-[800px]:block top-[47px] absolute left-[10px] z-30 rounded'>
			  ...
			  <div className={`${open ? 'block' : 'hidden'}  border-black px-[20px] py-[20px] rounded h-[400px]`}>
				  <div>
					  {ADMIN_SIDEBAR.map((s) => {
						  return (s.name == 'Profile' || s.name == 'WorkBoard') ?
